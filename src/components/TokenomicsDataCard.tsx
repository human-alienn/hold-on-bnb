...
export function TokenomicsDataCard({ icon, title, data, content, delay, extendedHeight = false, darkMode = false }: TokenomicsDataCardProps) {
  ...
  return (
    ...
    >
      ...
      <motion.div
        ...
        animate={{
          opacity: darkMode ? [0.15, 0.3, 0.15] : [0.4, 0.7, 0.4],
        }}
        transition={{
          duration: 3,
          repeat: Infinity,
      ...
      {/* Card content */}
      <motion.div
        className={`relative bg-black rounded-xl p-8 overflow-hidden h-full flex flex-col ${extendedHeight ? 'pb-16' : ''}`}
        animate={{
      ...
      >
        {/* Ambient glow */}
        <div
          className={`absolute top-0 right-0 w-32 h-32 rounded-full ${darkMode ? 'opacity-5' : 'opacity-20'}`}
          style={{
        ...
        {/* Sparkles on hover */}
        {!darkMode && Array.from({ length: 4 }).map((_, i) => (
          <motion.div
            key={i}
        ...
        {data && (
          <div className="mb-4 space-y-2">
            {data.map((item, i) => (
              <motion.div
                key={i}
                className="flex flex-col sm:flex-row sm:justify-between sm:items-center gap-1 sm:gap-4 py-2 border-b border-[#F3BA2F]/20"
              ...
              >
                ...
                <motion.span
                  className="text-[#F3BA2F] text-left sm:text-right"
                  animate={{
                    textShadow: [
        ...
        {/* Content / Quote */}
        <div className="text-white italic flex-1 flex flex-col justify-end" style={{ lineHeight: '1.8' }}>
          {content}
        </div>
        ...
        {/* Shimmer effect */}
        <motion.div
          className={`absolute inset-0 bg-gradient-to-r from-transparent ${darkMode ? 'via-[#F3BA2F]/3' : 'via-[#F3BA2F]/10'} to-transparent`}
          animate={{
            x: ['-100%', '200%'],
          }}
          transition={{
